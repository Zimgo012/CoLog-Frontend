...
import { ArrowLeftIcon, XMarkIcon, CameraIcon, ArrowsRightLeftIcon } from '@heroicons/react/24/outline'
...
export default function DocumentView() {
  ...
  return <div className="h-screen bg-base-200 flex flex-col overflow-hidden">
    <Navbar left={<><button onClick={() => navigate(`/diary/${id}/pages`, { state: { isOwner: isOwnerFromPages } })} className="btn btn-ghost btn-sm btn-circle" aria-label="Back"><ArrowLeftIcon className="w-5 h-5" /></button><span className="font-semibold text-sm">{pageId ? `Document ${pageId}` : 'Document'}</span></>} />
    <div className="flex flex-1 overflow-hidden"><main className="flex-1 overflow-y-auto"><div className="container mx-auto flex max-w-4xl flex-col gap-4 px-5 py-6 md:px-8 md:py-8">
      {wsError && <div role="alert" className="alert alert-warning text-sm">{wsError}</div>}
      <div className="overflow-hidden rounded-[2rem] bg-base-100 shadow-[0_12px_32px_oklch(var(--bc)/.07)]">{!user || !validDocument ? <div className="min-h-96 flex items-center justify-center"><span className="loading loading-spinner loading-md text-primary" /></div> : <CollabEditor diaryId={diaryId} documentId={documentId} userId={user.id} userName={`${user.firstName} ${user.lastName}`.trim() || user.username} onSaveSnapshot={handleSaveSnapshot} onSnapshotError={handleSnapshotError} onToggleHistory={() => setHistoryOpen(open => !open)} onDelete={() => setDeleteOpen(true)} historyOpen={historyOpen} />}</div>
    </div></main>
    {historyOpen && <aside className="z-50 flex w-80 shrink-0 flex-col border-l border-base-300 bg-base-200"><div className="flex items-center justify-between border-b border-base-300 px-5 py-4"><span className="text-sm font-black tracking-[-0.02em]">Revision history</span><button onClick={() => setHistoryOpen(false)} className="btn btn-ghost btn-xs btn-circle"><XMarkIcon className="w-4 h-4" /></button></div><div className="flex flex-1 flex-col gap-3 overflow-y-auto px-4 py-4"><p className="px-1 text-xs leading-relaxed text-base-content/45">Open a snapshot to preview or compare what changed.</p>{historyLoading && <span className="loading loading-spinner loading-sm text-primary self-center mt-4" />}{historyError && <div role="alert" className="alert alert-error text-xs py-2">{historyError}</div>}{!historyLoading && !historyError && revisions.length === 0 && <p className="py-4 text-center text-xs text-base-content/40">No snapshots yet.</p>}{revisions.map(revision => <button key={revision.revisionId} onClick={() => void openPreview(revision.revisionId)} className="rounded-2xl bg-base-100 p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md"><p className="flex items-center gap-1 text-xs font-bold"><CameraIcon className="h-3.5 w-3.5 text-primary" />{revisionTitle(revision)}</p><p className="mt-1 text-[11px] text-base-content/50">{formatDateTime(revision.createdAt)}</p>{revision.createdBy && <p className="text-[11px] text-base-content/40">by {revision.createdBy}</p>}</button>)}</div></aside>}</div>
